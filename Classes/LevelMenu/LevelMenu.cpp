...
void LevelMenu::update() {
    ...
    for (auto & item : option) {
        auto x = WIDTH / 2. - item.getGlobalBounds().width / 2;
        ...
        if (Mouse::isButtonPressed(Mouse::Left)) {
            switch(buttonPressed) {
                case 1:
                    //setController(std::make_unique<MainGame>(item.id));
                    break;
                case 2: Game::instance->setController(std::make_unique<MainMenu>()); break;
            }
        }
    }
}
