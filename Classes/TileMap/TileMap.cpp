...
bool TileMap::load(const std::string& tmx_file_path) {
    ...
    for (auto layer = root_element->FirstChildElement("layer"); layer != nullptr;
        ...
        for (size_t y = 0u, index = 0u; y < height; ++y) {
            for (size_t x = 0u; x < width; ++x, ++index) {
                ...
                if (tile_num) {   // обходим по часовой стрелке
                    ...
                    // определ€ем его текстурные координаты
    ...
    for (auto object_group = root_element->FirstChildElement("objectgroup");
        ...
        // получаем все данные - тип, имя, позицию, etc
        for (auto object = object_group->FirstChildElement("object");
            ...
            ObjectXML new_object(x, y, width, height);
...
ObjectXML TileMap::getObject(const std::string& name) {
    auto found = std::find_if(objects.begin(), objects.end(), [&](const ObjectXML& obj)
    {
...
std::vector<ObjectXML> TileMap::getObjectsByName(const std::string& name) {
    std::vector<ObjectXML> objects_by_name;

    std::copy_if(objects.begin(), objects.end(), std::back_inserter(objects_by_name), [&](const ObjectXML& obj)
    {
        return obj.name == name;
    });
...
// все объекты с заданным именем
std::vector<ObjectXML> TileMap::getObjectsByType(const std::string& type) {
    std::vector<ObjectXML> objects_by_type;

    std::copy_if(objects.begin(), objects.end(), std::back_inserter(objects_by_type), [&](const ObjectXML& obj)
    {
